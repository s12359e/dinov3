...
logger = logging.getLogger("train_pipeline_dist")
...
def setup_distributed():
    """Initialize distributed process group via torchrun env vars.

    Returns True if DDP was initialized, False if running single-GPU.
    """
    ...
    if rank != 0:
        logging.getLogger("train_pipeline_dist").setLevel(logging.WARNING)
...
class TrainTransform:
    """RandomResize -> RandomCrop -> HFlip -> VFlip -> PhotoMetric -> Normalize."""
...
class DiceLoss(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        target_oh = F.one_hot(target_clean, num_classes)
...
# ====================================================================
#  4. Metric -- Dice Score (with distributed reduce)
# ====================================================================

class DiceScoreMetric:
    """Accumulates intersection / area counts across batches then computes Dice.

    In distributed mode, reduces counts across all processes before computing.
    """
    ...
    def compute(self):
        ...
        if is_dist_initialized():
            for t in (self.intersection, self.pred_area, self.label_area):
                tensor_gpu = t.cuda()
                dist.all_reduce(tensor_gpu, op=dist.ReduceOp.SUM)
                t.copy_(tensor_gpu.cpu())

        dice = (2.0 * self.intersection) / (
            self.pred_area + self.label_area + 1e-8
        )
        result = {"mean_dice": dice.mean().item()}
        for c in range(self.num_classes):
...
def build_model(args, device):
    ...
    # SyncBN for multi-GPU, regular BN for single-GPU
    if is_dist_initialized():
...
def train(args):
    ...
    train_sampler = (
        DistributedSampler(train_ds, shuffle=True)
    ...
    # -- output dir (only rank 0 creates) -------------------------------
    ...
    for step in range(start_step + 1, args.max_iters + 1):
        ...
        if step % args.val_interval == 0:
            ...
            if is_best:
                ...
                if is_main_process():
                    _save_checkpoint(raw_model, optimizer, scheduler,
                                     step, best_dice,
                                     output_dir / "best.pth")
...
def parse_args():
    p = argparse.ArgumentParser(
        description="DINOv3 Segmentation Training (single-GPU / multi-GPU)",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Single GPU
  python train_pipeline_dist.py --data_root data/sem_defect --checkpoint ckpt.pth

  # 2 GPUs
  torchrun --nproc_per_node=2 train_pipeline_dist.py --data_root data/sem_defect --checkpoint ckpt.pth

  # 4 GPUs with options
  torchrun --nproc_per_node=4 train_pipeline_dist.py \\
      --data_root data/sem_defect --checkpoint ckpt.pth \\
      --msda_mode 4-level --use_pixel_shuffle --use_gated_attention \\
...
""",
    ...
    p.add_argument("--lr", type=float, default=1e-4,
                   help="Base learning rate (auto-scaled by world_size)")
    p.add_argument("--no_lr_scale", action="store_true",
                   help="Disable automatic LR scaling by world_size")
    ...
    # Auto-scale LR: linear scaling rule (lr * world_size)
    if not args.no_lr_scale and "WORLD_SIZE" in os.environ:
        world_size = int(os.environ["WORLD_SIZE"])
        original_lr = args.lr
        args.lr = args.lr * world_size
        # Log this after distributed setup (rank 0 only)
        args._lr_scaled_from = original_lr
        args._lr_scaled_world = world_size
...
if __name__ == "__main__":
    ...
    # Print LR scaling info (after setup_distributed is called inside train)
    if hasattr(args, "_lr_scaled_from"):
        # This will only print on rank 0 after setup
        print(f"[LR Scale] {args._lr_scaled_from:.2e} x {args._lr_scaled_world} GPUs "
              f"= {args.lr:.2e}")
